...
const DashboardRedirect: React.FC = () => {
	...
	useEffect(() => {
		...
		if (permittedRoute) {
			console.log(
				`🔄 Redirecting user ${user.name} to ${permittedRoute.route} based on ${permittedRoute.permission} permission`
			);
			navigate(permittedRoute.route);
		} else {
			// No permissions found, stay on current page (will show welcome message)
			console.log(
				`⚠️ User ${user.name} has no dashboard or primary permissions, showing welcome page`
			);
		}
	}, [user, navigate]);
