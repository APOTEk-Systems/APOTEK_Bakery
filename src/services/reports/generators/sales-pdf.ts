...
export const generateSalesPDF = (
	...
	try {
		...
		// Add summary row
		const totalSales = data.data.sales.reduce(
...
export const generateCreditSalesPDF = (
	...
	try {
		...
		const totalPaid = data.data.sales.reduce(
			(sum, sale) => sum + ((sale as any).paid || 0),
			0
		);
		const totalBalance = data.data.sales.reduce(
			(sum, sale) => sum + ((sale as any).outstandingBalance || 0),
			0
		);
		...
		autoTable(doc, {
			...
			didParseCell: function (data: any) {
				...
				if (
					data.section === 'body' &&
					data.row.index < tableData.length - 3 &&
					(data.column.index === 5 ||
						data.column.index === 6 ||
						data.column.index === 7)
				) {
