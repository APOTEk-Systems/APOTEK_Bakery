...
import {useToast} from "@/hooks/use-toast";
import {Search, Plus, Loader2} from "lucide-react"; // Removed Calendar
import {
  Table,
  TableBody,
  TableCell,
...
import {DateRangePicker, DateRange} from "@/components/ui/DateRange"; // Added DateRangePicker, DateRange
import {
  getInventory,
  getAdjustments,
  createAdjustment,
...
const SuppliesAdjustments = () => {
  const [dateRange, setDateRange] = useState<DateRange | undefined>(); // Changed from 'date' to 'dateRange' and undefined default
  const [adjustments, setAdjustments] = useState([]);
  const [inventory, setInventory] = useState<InventoryItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [dialogOpen, setDialogOpen] = useState(false);
  ...
  useEffect(() => {
    fetchInventory();
    fetchAdjustments();
  }, [dateRange]); // Dependency changed from 'date' to 'dateRange'
      ...
      <div className="p-6">
        <div className="mb-6">
          ...
          <div className="flex gap-4 mb-6">
            {/* Replaced single date input with DateRangePicker */}
            <DateRangePicker
              dateRange={dateRange}
            ...
            {adjustments.length === 0 && !loading && (
              <div className="text-center py-12">
                ...
                <p className="text-muted-foreground mb-4">
                  {dateRange // Changed from 'date' to 'dateRange'
